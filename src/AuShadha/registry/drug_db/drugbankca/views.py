    ...
    if request.method == 'GET' and request.is_ajax():
        try:
            ...
            print(
                ...
                active_ingredient)
            drugbankca_drug = DrugBankCaDrugs.objects.filter(
        ...
        if len(drugbankca_drug) == 0:
            ...
            drugbankca_drug = DrugBankCaDrugs.objects.filter(
                drug_name__iexact=active_ingredient)
            print(drugbankca_drug)
            if len(drugbankca_drug) == 0:
                drugbankca_drug = DrugBankCaDrugs.objects.filter(
                    drug_name__icontains=active_ingredient)
                if len(drugbankca_drug) == 0:
...
def drugbankcadrugs_search_by_drug_name(request):
    ...
    if request.method == "GET" and request.is_ajax():
        ...
        if drug_name == '*' or drug_name == ' ':
            ...
            drug_obj = DrugBankCaDrugs.objects.filter(
                drug_name__istartswith=drug_name)[:25]

        if len(drug_obj) == 0:
            drug_obj = DrugBankCaDrugs.objects.filter(
                drug_name__icontains=drug_name)[:25]
        data = []
        for d in drug_obj:
            data_to_append = {}
