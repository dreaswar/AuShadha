...
#           json_data['url'] = reverse('drugbankcadrugs_summary_by_drug_name',kwargs={'drug_name':drug.drug_name} )
                json_data['url'] = reverse('drugbankcadrugs_summary_by_drug_name') + "?drug_name=" + \
                    drug.drug_name + "&active_ingredient=" + drug.active_ingredient
        ...
        response['Content-Range'] = 'items' + \
            str(query_start) + '-' + str(query_end) + '/' + str(len(all_drugs))
        return response
    else:
    ...
    if request.method == 'GET' and request.is_ajax():
        ...
        try:
            ...
            return render_to_response(
                'registry/drug_db/fda_drugs/summary.html', variable)
...
def fda_drug_db_search(request):
    ...
    if request.method == 'GET':
        if search_for == '*' or search_for == ' ':
            ...
            drugs = FDADrugs.objects.filter(
                drug_name__istartswith=search_for)[:25]
        data = []
        print drugs
        for d in drugs:
            data_to_append = {}
...
def fda_drug_db_advanced_search(request):
    ...
    if request.method == 'GET' and request.is_ajax():
        ...
        return render_to_response(
            'drug_db/fda_drugs/advanced_search.html', variable)
        ...
        if filter_by and search_for:
            ...
            if drugs:
                for d in drugs:
                    ...
                    data_to_append['absolute_url'] = d.get_absolute_url()
                    data_to_append[
                        'active_ingredient'] = '%s - %s as %s ' % (d.drug_name.title(), d.dosage, d.form)
                    data.append(data_to_append)
        jsondata = json.dumps(data)
        return HttpResponse(jsondata, content_type='application/json')
    else:
