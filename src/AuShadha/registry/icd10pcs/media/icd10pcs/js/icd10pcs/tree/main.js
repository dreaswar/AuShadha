...
function(win,
    ...
    var treeContainerBcDomId = 'ICD10PCS_CENTER_BC';
    var codeGeneratorDomId = 'ICD10PCS_BOTTOM_CP';
    var treeDomIds = ['ICD10PCS_CP_1_TREE','ICD10PCS_CP_2_TREE','ICD10PCS_CP_3_TREE'];
    var pcsRowDomId = 'ICD10PCS_CP_4';
    ...
    function onTreeDblClick(item) {
		...
		for (var i=0; i<treeDomIds.length; i++) { 
		       ...
		       domConstruct.create(target_node_type, { id: treeDomIds[i] }, parent_dom, 0 );
                ...
                // This is a lame code generator. This will be moved to a separate module
                var code = item.name[0].split('-')[0];
                registry.byId(codeGeneratorDomId).set('content', "Generated Code: " + code); 
        } 
