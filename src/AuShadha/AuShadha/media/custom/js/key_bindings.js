    ...
    on(document.body, "keyup", function (e) {
        ...
        function createDomsAndDijits(action, urlStore){
              ...
              if(registry.byId('addChoiceFilteringSelect')){
                registry.byId('addChoiceFilteringSelect').destroyRecursive();
              }
              if(registry.byId('addChoiceSubmitButton')){
                registry.byId('addChoiceSubmitButton').destroyRecursive();
              }

              domConstruct.create("div", {id: "addDialogContentDiv"},"addDialogContent","first");
              domConstruct.create("div", {id: "addChoiceFilteringSelect"},"addDialogContentDiv",0);
              domConstruct.create("input", {id: "addChoiceSubmitButton",
              ...
              var filteringSelect = new FilteringSelect({
                                                        ...
                                                        onBlur       : function(e){
                                                                          ...
                                                                          if(urlToCall){
                                                                            ...
                                                                            registry.byId('addChoiceSubmitButton').focus();
                                                                          }
                                                                          else{
                                                                            return false;
                                                                          }
                                                        ...
                                                        if(urlToCall){
                                                          request(urlToCall).then(
                                                            function(html){
                                                              ...
                                                              if(urlToCall == CHOSEN_PATIENT.contactjson || 
                                                                 urlToCall  == CHOSEN_PATIENT.phonejson || 
                                                                 urlToCall  == CHOSEN_PATIENT.guardianjson || 
                                                                 urlToCall  == CHOSEN_PATIENT.admissionjson || 
                                                                 urlToCall  == CHOSEN_PATIENT.visitjson ||
                                                                 urlToCall  == CHOSEN_PATIENT.medicationlistjson ||
                                                                 urlToCall  == CHOSEN_PATIENT.allergiesjson ||
                                                                 urlToCall  == CHOSEN_PATIENT.medicalhistoryjson ||
                                                                 urlToCall  == CHOSEN_PATIENT.surgicalhistoryjson ||
                                                                 urlToCall  == CHOSEN_PATIENT.familyhistoryjson ||
                                                                 urlToCall  == CHOSEN_PATIENT.immunisationjson 
                                                              ){
                                                                console.log("Trying to create a Grid Popup...")
                                                                var storeItem = urlStore.get(urlToCall);
                                                                var gridStr  = storeItem.gridStr;
                                                                var gridHtml = storeItem.gridFn(urlToCall, gridStr ,'gridDialogContent');
                                                            ...
                                                            function(error){
                                                              ...
                                                              registry.byId('addChoiceFilteringSelect').set('displayedValue','');
                                                              registry.byId('addChoiceFilteringSelect').focus();
                                                            }
                                                          );
                                                        }
                                                        else{
             ...
             registry.byId("addDialog").set('title',action);
             registry.byId("addDialog").show();
        }
          ...
          if (CHOSEN_PATIENT){
            ...
            if( registry.byId('patientHomeContentPane').get('selected')   || 
              ...
              var addUrlStore = new Memory({ data: [
                                                ...
                                                //{% if perms.patient.add_patientcontact %} //
                                                {name:"Contact"              , id:CHOSEN_PATIENT.contactadd},
                                                //{% endif %}//

                                                //{% if perms.patient.add_patientphone %}
                                                {name:"Phone"                , id:CHOSEN_PATIENT.phoneadd},
                                                //{% endif %}

                                                //{% if perms.patient.add_patientguardian %}
                                                {name:"Guardian"             , id:CHOSEN_PATIENT.guardianadd},
                                                //{% endif %}

                                                //{% if perms.admission.add_admission %}
                                                {name:"Admission"            , id:CHOSEN_PATIENT.admissionadd},
                                                //{% endif %}
                                                
                                                //{% if perms.visit.add_visitdetail %}
                                                {name:"Visit"                , id:CHOSEN_PATIENT.visitadd},
                                                //{% endif %}
                                           
                                                //{% if perms.patient.add_patientmedicationlist %}
                                                {name:"Medication List"      , id:CHOSEN_PATIENT.medicationlistadd},
                                                //{% endif %}

                                                //{% if perms.patient.add_patientallergy %}
                                                {name:"Allergy"              , id:CHOSEN_PATIENT.allergiesadd},
                                                //{% endif %}
                                                ...
                                                //{% if perms.patient.add_patientfamilyhistory %}
                                                {name:"Family History"       , id:CHOSEN_PATIENT.familyhistoryadd},
                                                //{% endif %}

                                                //{% if perms.patient.add_patientsocialhistory %}
                                                {name:"Social History"       , id:CHOSEN_PATIENT.socialhistoryadd},
                                                //{% endif %}

                                                //{% if perms.patient.add_patientmedicalhistory %}
                                                {name:"Medical History"      , id:CHOSEN_PATIENT.medicalhistoryadd},
                                                //{% endif %} 

                                                //{% if perms.patient.add_patientimmunisation %}
                                                {name:"Immunisation History" , id:CHOSEN_PATIENT.immunisationadd},
                                                //{% endif %}

                                                //{% if perms.patient.add_patientsurgicalhistory %}
                                                {name:"Surgical History"     , id:CHOSEN_PATIENT.surgicalhistoryadd},
                                                //{% endif %}

                                                //{% if perms.obs_and_gyn.add_obstetrichistorydetail %}
                                                {name:"Obstetric History"    , id:CHOSEN_PATIENT.obstetrichistorydetailadd},
                                                //{% endif %}

                                                //{% if perms.patient.patientdemographics %}
                                                {name:"Demographics"         , id:CHOSEN_PATIENT.demographicsadd}
                                                //{% endif %}
                                            ]
                                });
