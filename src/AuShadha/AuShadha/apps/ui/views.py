...
def home(request):
    ...
    if request.method == "GET":
        ...
        variable = RequestContext(
            request, {
                'user': user, 'installed_apps': installed_apps})
...
def get_reference_apps(request):
    if request.method == 'GET':
        ...
        data = {
            'success': True, 'apps': [
                app._meta.app_label.replace(
                    '_', ' ').upper() for app in apps]}
...
def serialise_ui(UI):
    ...
    for k, v in registry.items():
        dict_to_append = {
            'role': k, 'class': v.__name__ if isinstance(
                v, str) else unicode(v)}
...
def installed_apps(request):
    ...
    if request.method == "GET":
        ...
        for app in apps:
            ...
            # Hack to avoid core modules. This way the UI atleast starts with core modules
            # as dependencies
            if main_module not in ['django',
                                   'AuShadha'] or app_name in ['search']:
                x = importlib.import_module(app)
                label = getattr(x, 'MODULE_LABEL', None)
