...
class Clinic(AuShadhaBaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._extra_url_actions = [
            'transfer_patient', 'transfer_clinic', 'refer']

    name_of_clinic = models.CharField(max_length=200)
    nature_of_clinic = models.CharField(
        max_length=200, choices=CLINIC_NATURE_CHOICES)
...
class Phone(AuShadhaBaseModel):
    ...
    def __unicode__(self):
        return '%s-%s-%s' % (self.country_code,
                             self.area_code, self.phone_number)
...
#    staff_detail  = models.ForeignKey(AuShadhaUser)
    clinic_staff_role = models.CharField(
        "Staff Role",
        max_length=100,
    ...
    is_staff_hod = models.BooleanField(
        "Is Staff Head of the Department", default=None)
