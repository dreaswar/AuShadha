...
class DijitTreeNode(object):
    ...
    def __init__(self, attrs):
        ...
        if self.check_node_attr(attrs):
            ...
            for k, v in attrs.iteritems():
                if k == 'id' and v not in self.id_registry:
    ...
    def is_id_unique(self, attrs):
        id_val = attrs['id']
        if id_val in self.id_registry:
            raise Exception("DuplicateIDError: " +
                            id_val +
                            '. Registry already has following keys: ' +
                            str(self.id_registry.keys()))
        else:
    ...
    def check_for_basic_attrs(self, attrs):
        for attr in self.basic_tree_attrs:
            if attr in attrs:
                ...
                raise Exception(
                    "Basic Attributes Not Supplied while creating a tree")
        return True
