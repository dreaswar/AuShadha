...
class SingleModelInstanceView(object):
  """Creates a Class Based Representation of Single Model Instance 
     for manipulation and HTML Formatting
  ...
  def get_template(self, template_action):
    """ Gets Template with a given action and Path. If not set Returns None """
    try:
  ...
  def template_render(self,template_action,request):
    try:
      ...
      if t and c: 
        ...
        raise Exception("Template / Request Value Cannot be Null. Nothing to Render!")
    except('TemplateDoesNotExist'):
  ...
  def build_html_div(self):
    ...
    Custom CSS classes Validation can be done here using self._eval for each value
    If you dont want to use this way of HTML building there is always the 
    self.template_render method
    """
    pass
