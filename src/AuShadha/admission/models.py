...
class Admission(models.Model):
    ...
    clinic = models.CharField(
        max_length=30, default="Ortho One")
    room_or_ward = models.CharField(
        max_length=30, blank=True, null=True)
    patient_detail = models.ForeignKey('patient.PatientDetail')
    admitting_physician = models.ForeignKey(
        'physician.PhysicianDetail', related_name='admitting_physician')
    ...
    class Meta:
        ...
        ordering = ('date_of_admission', 'time_of_admission',
                    'patient_detail', 'admitting_physician')
...
#
    def get_absolute_url(self):
        ...
        This webpage will handle all the current admission related
        events.
...
#
    def get_phy_exam_main_window(self):
        """Returns the URL of the Physical Examination List for this
        admission."""
...
#
    def has_phy_exam(self):
        """Returns whether a chosen Admission Object has already a Physical
        Exam Recorded for Discharge Summary."""
    ...
    def has_complaints(self):
        ...
        adm_complaints = AdmissionComplaint.objects.filter(
    ...
    def complaints_for_adm(self):
        ...
        if not adm_obj.has_complaints():
            ...
            adm_complaint_obj = AdmissionComplaint.objects.filter(
                admission_detail=adm_obj)
    ...
    def has_past_history(self):
        id = self.id
        adm_obj = Admission.objects.get(pk=id)
        adm_past_history = AdmissionPastHistory.objects.filter(
            admission_detail=adm_obj)
        if adm_past_history:
            return True
        else:
    ...
    def has_diag(self):
        ...
        try:
            ...
            return False
        adm_diag_obj = AdmissionDiagnosis.objects.filter(
            admission_detail=adm_obj)
        if adm_diag_obj:
            return True
        else:
    ...
    def has_imaging(self):
        id = self.id
        adm_obj = Admission.objects.get(pk=id)
        adm_imaging = AdmissionImaging.objects.filter(
            admission_detail=adm_obj)
        if adm_imaging:
            return True
        else:
    ...
    def has_inv(self):
        id = self.id
        adm_obj = Admission.objects.get(pk=id)
        adm_inv = AdmissionInv.objects.filter(
            admission_detail=adm_obj)
        if adm_inv:
            return True
        else:
    ...
    def has_exam_on_admission(self):
        ...
        try:
            ...
            return False
        phy_exam_obj = PhyExam.objects.filter(
            admission_detail=adm_obj).filter(consult_nature='initial')
        if phy_exam_obj:
            return True
        else:
            return False
    ...
    def has_exam_on_preop(self):
        ...
        if self.has_phy_exam() != '0':
            phy_exam_obj = PhyExam.objects.filter(
                admission_detail=adm_obj).filter(consult_nature='pre_op')
            if phy_exam_obj:
                return True
            else:
                return False
    ...
    def has_exam_on_postop(self):
        ...
        if self.has_phy_exam() != '0':
            phy_exam_obj = PhyExam.objects.filter(
                admission_detail=adm_obj).filter(consult_nature='post_op')
            if phy_exam_obj:
                return True
            else:
                return False
    ...
    def has_exam_on_discharge(self):
        ...
        if self.has_phy_exam() != '0':
            phy_exam_obj = PhyExam.objects.filter(
                admission_detail=adm_obj).filter(consult_nature='discharge')
            if phy_exam_obj:
                return True
            else:
                return False
    ...
    def get_admission_discharge(self):
        ...
        if self.admission_closed:
            id = self.id
            discharge_obj = DischargeDetail.objects.filter(
                admission_detail=self)
            if discharge_obj:
    ...
    def fit_for_discharge_check(self, user, date_of_discharge):
        """Checks to see if the admission is fit for Discharge.
...
#
    ...
    def return_phy_exam_model_form(self, phy_exam_instance, consult_nature, request_post=False):
        ...
        if len(phy_exam_obj) == 0:
            ...
            if not request_post:
                return IP_Initial_PhyExamForm(instance=phy_exam_instance)
            else:
                return IP_Initial_PhyExamForm(request_post, instance=phy_exam_instance)
            ...
            if consult_nature == 'fu':
                ...
                if not request_post:
                    ...
                    form = IP_Fu_PhyExamForm(
                        request_post, instance=phy_exam_instance)
                    return form
