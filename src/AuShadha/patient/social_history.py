        ...
        if request.method == "GET" and request.is_ajax():
            try:
                ...
                if social_history_obj:
                    ...
                    patient_social_history_obj = PatientSocialHistory(
                        patient_detail=patient_detail_obj)
                    patient_social_history_form = PatientSocialHistoryForm(
                        instance=patient_social_history_obj)
            ...
            try:
                ...
                copy_post['pets'] = ",".join(
                    copy_post.getlist('pets'))
                patient_social_history_form = PatientSocialHistoryForm(
                    copy_post, instance=patient_social_history_obj)
...
def patient_social_history_edit(request, id):
    ...
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            ...
            try:
                id = int(id)
                social_history_obj = PatientSocialHistory.objects.get(
                    pk=id)
                patient_social_history_form = PatientSocialHistoryForm(
                ...
                raise Http404(
                    "BadRequest: Patient SocialHistory Data Does Not Exist")

        elif request.method == 'POST' and request.is_ajax():

            try:
                id = int(id)
                patient_social_history_obj = PatientSocialHistory.objects.get(
                    pk=id)
                copy_post = request.POST.copy()
                copy_post['home_occupants'] = ",".join(
                    copy_post.getlist('home_occupants'))
