    ...
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            try:
                ...
                patient_immunisation_obj = PatientImmunisation(
                    patient_detail=patient_detail_obj)
                patient_immunisation_add_form = PatientImmunisationForm(
                    instance=patient_immunisation_obj)
...
#        raise Http404("BadRequest")
            ...
            try:
                ...
                patient_immunisation_add_form = PatientImmunisationForm(
                    request.POST, instance=patient_immunisation_obj)
...
def patient_immunisation_edit(request, id):
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            try:
                id = int(id)
                patient_immunisation_obj = PatientImmunisation.objects.get(
                    pk=id)
                patient_immunisation_edit_form = PatientImmunisationForm(
                    instance=patient_immunisation_obj)
                variable = RequestContext(request,
                                          {"user": user,
    ...
    if request.user and user.is_superuser:
        if request.method == "GET":
            try:
                ...
                raise Http404(
                    "BadRequest: Patient immunisation Data Does Not Exist")
