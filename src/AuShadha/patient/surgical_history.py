...
def surgical_history_json(request):
    try:
        ...
        patient_surgical_history_obj = PatientSurgicalHistory.objects.filter(
    ...
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            try:
                id = int(id)
                patient_detail_obj = PatientDetail.objects.get(pk=id)
                patient_surgical_history_obj = PatientSurgicalHistory(
                    patient_detail=patient_detail_obj)
                patient_surgical_history_add_form = PatientSurgicalHistoryForm(
                    instance=patient_surgical_history_obj)
            ...
            try:
                ...
                patient_surgical_history_obj = PatientSurgicalHistory(
                    patient_detail=patient_detail_obj)
                patient_surgical_history_add_form = PatientSurgicalHistoryForm(
                    request.POST, instance=patient_surgical_history_obj)
...
def patient_surgical_history_edit(request, id):
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            try:
                id = int(id)
                patient_surgical_history_obj = PatientSurgicalHistory.objects.get(
                    pk=id)
                patient_surgical_history_edit_form = PatientSurgicalHistoryForm(
                    instance=patient_surgical_history_obj)
                variable = RequestContext(request,
                                          {"user": user,
    ...
    if request.user and user.is_superuser:
        if request.method == "GET":
            try:
                ...
                raise Http404(
                    "BadRequest: Patient surgical_history Data Does Not Exist")
