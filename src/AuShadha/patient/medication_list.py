  ...
  if request.user:
    ...
    if request.method =="GET" and request.is_ajax():
      try:
        ...
        patient_medication_list_obj       = PatientMedicationList(patient_detail = patient_detail_obj)
        patient_medication_list_add_form  = PatientMedicationListForm(instance = patient_medication_list_obj)
...
#        raise Http404("BadRequest")
      ...
      try:
        ...
        patient_detail_obj      = PatientDetail.objects.get(pk =id)
        patient_medication_list_obj       = PatientMedicationList(patient_detail = patient_detail_obj)
        patient_medication_list_add_form  = PatientMedicationListForm(request.POST,instance = patient_medication_list_obj)
...
def patient_medication_list_edit(request,id):
  if request.user:
    ...
    if request.method =="GET" and request.is_ajax():
      try:
        id                            = int(id)
        patient_medication_list_obj         = PatientMedicationList.objects.get(pk = id)
        patient_medication_list_edit_form   = PatientMedicationListForm(instance = patient_medication_list_obj)
        variable                      = RequestContext(request, 
        																					{"user" 									      :	user,
      ...
      try:
        id                            = int(id)
        patient_medication_list_obj         = PatientMedicationList.objects.get(pk = id)
        patient_medication_list_edit_form   = PatientMedicationListForm(request.POST,instance = patient_medication_list_obj)
  ...
  if request.user and user.is_superuser:
    if request.method =="GET":
       try:
          ...
          raise Http404("BadRequest: Patient medication_list Data Does Not Exist")
