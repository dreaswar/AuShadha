...
def generic_url_maker(instance, action, id, root_object=False):
    ...
    if not root_object:
        url = unicode(APP_ROOT_URL) + unicode(instance._meta.app_label) + \
...
class PatientDetail(models.Model):
    ...
    """Patient Detail Model that gives the models definition and the associated
    methods to control the Patient models."""
...
# If this returns false, the save should fail raising proper Exception
    ...
    def get_patient_home_url(self):
        ...
        This is the place where the central actions of the Patient can
        be managed at one place. This includes Contacts/ Admissions / OP
        visits etc..
    ...
    def has_guardian(self):
        ...
        class Meta:
            ...
            ordering = ('first_name', 'middle_name',
                        'last_name', 'age', 'sex', 'patient_hospital_id')
...
class PatientEmailFax(models.Model):
    ...
    """Model that manages the Email, Fax and Web contact details of a
    patient."""
...
class PatientDemographicsData(models.Model):
    ...
    """Maintains Demographic, Social and Family data for the patient This has
    been adapted from the excellent work by GNU Health project However this is
    a very rudimentary adaptation."""
...
class PatientAllergies(AuShadhaBaseModel):
    ...
    This defines the Allergies that the patient has. The patient
    automatically belongs to a Clinic and has some add, edit, del
...
class PatientImmunisation(AuShadhaBaseModel):
    ...
    This defines the Immunisation that the patient has had. The patient
    automatically belongs to a Clinic and has some add, edit, del
    methods defined on him.
...
class VaccineRegistry(models.Model):
    ...
    """Registry for the Vaccines This contains the details of vaccine, VIS,
...
class PatientFamilyHistory(AuShadhaBaseModel):
    ...
    The patient automatically belongs to a Clinic and has some add,
    edit, del methods defined on him.
...
class PatientSocialHistory(AuShadhaBaseModel):
    ...
    This defines the Social History that the patient has had. The
...
class PatientMedicalHistory(AuShadhaBaseModel):
    ...
    This defines the Medical History that the patient has had. The
    patient automatically belongs to a Clinic and has some add, edit,
    del methods defined on him.
    ...
    pregnancy_warning = models.ForeignKey(
        'obs_and_gyn.ObstetricHistoryDetail',
        blank=True,
        null=True
    )
    ...
    def get_edit_url(self):
        """Returns the URL for editing Medical History details for a
        Patient."""
        return '/AuShadha/pat/%s/edit/%s/' % (self.__model_label__, self.id)

    def get_del_url(self):
        """Returns the URL for deleting Medical History detail for a
        Patient."""
        return '/AuShadha/pat/%s/del/%s/' % (self.__model_label__, self.id)
...
class PatientSurgicalHistory(AuShadhaBaseModel):
    ...
    This defines the Surgical History that the patient has had. The
    patient automatically belongs to a Clinic and has some add, edit,
    del methods defined on him.
    ...
    def get_edit_url(self):
        """Returns the URL for editing Surgical History details for a
        Patient."""
        return '/AuShadha/pat/%s/edit/%s/' % (self.__model_label__, self.id)

    def get_del_url(self):
        """Returns the URL for deleting Surgical History detail for a
        Patient."""
        return '/AuShadha/pat/%s/del/%s/' % (self.__model_label__, self.id)
