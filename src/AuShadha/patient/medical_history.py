...
def medical_history_json(request):
    try:
        ...
        patient_medical_history_obj = PatientMedicalHistory.objects.filter(
    ...
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            try:
                id = int(id)
                patient_detail_obj = PatientDetail.objects.get(pk=id)
                patient_medical_history_obj = PatientMedicalHistory(
                    patient_detail=patient_detail_obj)
                patient_medical_history_add_form = PatientMedicalHistoryForm(
                    instance=patient_medical_history_obj)
            ...
            try:
                ...
                patient_medical_history_obj = PatientMedicalHistory(
                    patient_detail=patient_detail_obj)
                patient_medical_history_add_form = PatientMedicalHistoryForm(
                    request.POST, instance=patient_medical_history_obj)
...
def patient_medical_history_edit(request, id):
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            try:
                id = int(id)
                patient_medical_history_obj = PatientMedicalHistory.objects.get(
                    pk=id)
                patient_medical_history_edit_form = PatientMedicalHistoryForm(
                    instance=patient_medical_history_obj)
                variable = RequestContext(request,
                                          {"user": user,
    ...
    if request.user and user.is_superuser:
        if request.method == "GET":
            try:
                ...
                raise Http404(
                    "BadRequest: Patient Medical History Data Does Not Exist")
