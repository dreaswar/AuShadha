...
def render_patient_tree(request,id = None):
  if request.method=="GET" and request.is_ajax():
    if id:
      ...
      immunisation_obj      = PatientImmunisation.objects.filter(patient_detail = pat_obj)

      medication_list_obj   = PatientMedicationList.objects.filter(patient_detail = pat_obj)
      allergies_obj           = PatientAllergies.objects.filter(patient_detail = pat_obj)
      ...
      data = {
         ...
         "label"     : "name" ,
         "items"     : [
                          {"name"  : "Synopsis"   , "type":"application", "id":"SYNOPSIS",
                           'len'   : 1,
                           "addUrl": None,
                          },
                          {"name"    : "Front Page"   , "type":"application", "id":"FRONT_PAGE",
                           'len'     : 1,
                           "addUrl"  : None,
                           'children':[ 
      ...
      def generic_tree_builder(obj_list, index, type_label):
        if not obj_list:
          ...
          children_list                    = data['items'][index]['children']
          dict_to_append                   = {"name"   : "",
                                              "type"   : type_label,
                                              "id"     : "",
                                              "editUrl": "",
      ...
      for obj_list in history_obj_list:
        generic_tree_builder(obj_list, obj_list_index, obj_list_label_mapper[obj_list])
        obj_list_index += 1
...
def visit_json(request):
  ...
  if patient_visit_obj:
    for visit in patient_visit_obj:
      ...
      data_to_append['visit_date']       = visit.visit_date.date().isoformat() + i
