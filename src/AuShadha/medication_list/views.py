...
def medication_list_json(request, patient_id=None):
    try:
        ...
        jsondata = generate_json_for_datagrid(medication_list_obj)
...
def medication_list_add(request, patient_id=None):
    ...
    if request.user:
        ...
        if request.method == "GET" and request.is_ajax():
            medication_list_form = MedicationListForm(
                instance=medication_list_obj, auto_id=False)
            ...
            if medication_list_form.is_valid():
                ...
                fields_list = [
                    field for field in medication_list_obj._meta.fields if field.serialize]
                ...
                addData = {f.name: f.value_to_string(
                    medication_list_obj) for f in fields_list}
            ...
            try:
                if medication_list_id:
                    ...
                    medication_list_id = int(
                        request.GET.get('medication_list_id'))
                medication_list_obj = MedicationList.objects.get(
                    pk=medication_list_id)
                patient_detail_obj = medication_list_obj.patient_detail
            except TypeError or ValueError or AttributeError:
