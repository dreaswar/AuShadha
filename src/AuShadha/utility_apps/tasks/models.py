...
class TaskDetail(AuShadhaBaseModel):
    ...
    def is_delayed(self):
        ...
        if not _now > self.deadline and self.status not in [
                'completed', 'cancelled', 'deferred']:
            return True
        return

    def is_active(self):
        if self.status not in ['completed', 'cancelled', 'deferred']:
            return True
        return
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            form_fields = open(
                os.path.abspath(
                    os.path.curdir) +
                '/utility_apps/tasks/dijit_fields_constants.yaml')
...
class TaskDetailForm(AuShadhaBaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dijit_fields = YAMLToAuFormLoader().__call__()[
