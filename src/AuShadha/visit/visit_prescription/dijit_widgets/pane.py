...
def render_visit_prescription_pane(request, visit_detail_id = None):
  ...
  if request.method == 'GET' and request.is_ajax():
    try:
      ...
      context = Context({'visit_detail_id': visit_detail_id, 'visit_detail_obj': visit_detail_obj })
      
      try:
        pane_template = Template( open('visit/visit_prescription/dijit_widgets/pane.yaml','r').read() )
      except( IOError ):
