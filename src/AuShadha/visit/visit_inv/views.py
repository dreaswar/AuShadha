    ...
    if request.method == "GET" and request.is_ajax():
        ...
        d = {
            ...
            'all_visits': VisitDetail.objects.filter(
                patient_detail=patient_detail_obj),
            'inactive_visits': VisitDetail.objects.filter(
                patient_detail=patient_detail_obj).filter(
                is_active=False)}
    ...
    if request.method == "GET" and request.is_ajax():
        ...
        if visit_detail_obj:
            ...
            for visit in visit_detail_obj:
                ...
                if vasc_exam_obj:
                    vasc_f = vascexamobjpresentationclass_querysetfactory(
                        vasc_exam_obj)
                else:
...
def visit_detail_add(request, patient_id=None, nature='initial'):
    ...
    try:
        ...
        initial_data = [{'location': "Dorsalis Pedis",
                         'side': 'right', 'character': 'normal'}]
    except (TypeError, NameError, ValueError, AttributeError, KeyError):
        raise Http404("Error ! Invalid Request Parameters. ")
    except (PatientDetail.DoesNotExist):
        raise Http404("Requested Patient Does not exist.")
    ...
    if not patient_detail_obj.can_add_new_visit(patient_detail_obj):
        ...
        VascExam_FreeModelFormset = modelformset_factory(
            VascExam_FreeModel,
        ...
        complaint_form_var_dict = {
            'prefix': visit_complaint_formset_prefix,
            'total_form_id': visit_complaint_formset_prefix + "-TOTAL_FORMS",
            'form_count': ''}  # Ultimately this may be better way to pass the vars to template
...
def visit_detail_edit(request, visit_id=None):
    ...
    if request.method == "GET" and request.is_ajax():
        ...
        visit_complaint_formset_prefix = "edit_visit_complaints_" + \
            str(visit_id)
        ...
        if vasc_exam_free_model_obj:
            ...
            VascExam_FreeModelFormset = modelformset_factory(
                VascExam_FreeModel, form=VascExam_FreeModelForm, extra=0)
            ...
            VascExam_FreeModelFormset = modelformset_factory(
                VascExam_FreeModel, form=VascExam_FreeModelForm, extra=1)
            ...
            # auto_id=vasc_exam_form_auto_id)
        vasc_exam_free_model_formset = VascExam_FreeModelFormset(
            queryset=VascExam_FreeModel.objects.filter(
                visit_detail=visit_detail_obj),
            auto_id=vasc_exam_form_auto_id,
            prefix=vasc_exam_formset_prefix)
        generic_add_icon_html = generic_table_form_add_icon_template.render(
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        error_message = None
        VascExam_FreeModelFormset = modelformset_factory(
            VascExam_FreeModel, form=VascExam_FreeModelForm)
        if visit_complaint_obj and visit_hpi_obj and visit_ros_obj:
            ...
            if vasc_exam_free_model_obj:
                ...
                vasc_exam_formset_auto_id = "id_" + \
                    vasc_exam_formset_prefix + "_edit_vasc_exam_" + str(id)
                ...
                # vasc_exam_free_model_obj = VascExam_FreeModel(
                    # visit_detail=visit_detail_obj)
                vasc_exam_formset_auto_id = "id_" + \
                    vasc_exam_formset_prefix + "_add_vasc_exam_" + str(id)
                vasc_exam_total_form_auto_id = "id_" + vasc_exam_formset_prefix +\
        ...
        if request.method == "GET" and request.is_ajax():
            ...
            visit_follow_up_form = VisitFollowUpForm(
                ...
                initial={
                    ...
                    'op_surgeon': visit_detail_obj.op_surgeon,
                    'consult_nature': 'follow_up'
                }
            )
...
def visit_follow_up_edit(request, follow_up_id=None):
    ...
    try:
        ...
        if not getattr(visit_follow_up_obj, 'urls', None):
            visit_follow_up_obj.save()
        ...
        if not visit_follow_up_obj.visit_detail.is_active:
            raise Http404("The Visit is Closed. Cannot add Followup Visits")
        ...
        if request.method == "GET" and request.is_ajax():
            ...
            visit_follow_up_form = VisitFollowUpForm(
                instance=visit_follow_up_obj,
                auto_id='id_visit_follow_up_' + str(
                    visit_follow_up_obj.id) + "_%s",
                initial={
                    'visit_date': visit_follow_up_obj.visit_date.isoformat()})
        ...
        if user.has_perm('visit.delete_visitfollowup'):
            try:
                if follow_up_id:
                    ...
                    follow_up_id = int(request.GET.get('follow_up_id'))
                visit_follow_up_obj = VisitFollowUp.objects.get(
                    pk=follow_up_id)
            except (TypeError, NameError, ValueError, AttributeError, KeyError):
                raise Http404("Error ! Invalid Request Parameters. ")
            except (VisitFollowUp.DoesNotExist):
...
# Creates a significant additional skillset to learn with a very limited
# use case
    ...
    if request.user:
        ...
        if request.method == 'GET':
            ...
            return render_to_response(
                'visit_detail/visit_pdf_template.html', variable)
