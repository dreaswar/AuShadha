...
class PhyExamBasePresentationClass(object):
        ...
        for field in self._meta.fields:
            try:
                ...
                try:
                    ...
                    # print "Calling Factory Validator with Compare with: ",
                    # f_compare_with
    ...
    def build_html_table(self):
        ...
        for v in self.field_map.values():
            ...
            row += (line + '</tr>')
            body += row
        return """<table class='content_pane_table'> %s %s </table>""" % (
            header, body)
...
def vascexamobjpresentationclass_querysetfactory(queryset):
    ...
    # print h_rows
    return "<table class='content_pane_table'> %s <tbody> %s </tbody> </table>" % (
        h_head, h_rows)
